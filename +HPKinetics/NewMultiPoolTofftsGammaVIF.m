            ...
            % first step
             ...
             %v1 = Mxy %should be the "boundary condition" but it may not be used
              ...
              if MDAnderson2023REU
                  ...
                  for i = 2:N
                      j = 3*i; %For the TR_list, will change to pull from TR_List later
                      v_star = C.*expm(-1*A*j).*ve + kve.'/ve .*A_inv.*(I - expm(-1.*A.*j)).*MDfunc(j) %New, suppress later; changed VIF(:, i) -> MDfunc(i)
                      v1 = C.*expm(-1*A*j).*ve + kve.'/ve .*A_inv.*(I - expm(-1.*A.*j)).*MDfunc(j) + A_inv^2./j .*(j.*A - I + expm(-1.*A.*j)).*(MDfunc(v_star)- MDfunc(Mz(:, i-1))) %New, "i-1" on MDfunc(v1(i-1) kinda iffy
